...
public class ViewCreationSpecTest {
  ...
  public void testViewGeneratorSpecParser() {
    ...
    // Test PinotTableSpec
    final ViewCreationSpec.PinotTableSpec pinotTableSpec =
        PinotUtils.getPinotTableSpecFromViewGenerationSpec(viewCreationSpec);
    ...
    Assertions.assertEquals(
        ...
        List.of(
            ...
            "properties__KEYS",
            "properties__VALUES"));
    Assertions.assertEquals(pinotTableSpec.getMetricColumns(), List.of("time_taken_millis"));
    Assertions.assertEquals(pinotTableSpec.getColumnsMaxLength(), Map.of("id_sha", 64));
    Assertions.assertEquals(
    ...
    Assertions.assertEquals(
        ...
        "http://localhost:8081");
    Assertions.assertEquals(
        streamConfigs.get("stream.kafka.hlc.zk.connect.string"), "localhost:2181");
    ...
    Assertions.assertEquals(
        streamConfigs.get("stream.kafka.consumer.prop.auto.offset.reset"), "largest");
  }
  ...
  public void testAvroDecoderSchemaIsAutoSetInTableCreationRequest() {
    ...
    final Map<String, Object> streamConfigs =
        PinotUtils.getPinotTableSpecFromViewGenerationSpec(viewCreationSpec).getStreamConfigs();
    Assertions.assertEquals(
  ...
  public void testKafkaSpecToString() {
    ...
    Assertions.assertEquals(
        "Topic Name: test-topic, Partitions: 3, ReplicationFactor: 10, BrokerAddress: broker:9000",
        kafkaSpec.toString());
    ...
    Assertions.assertEquals(
        "Topic Name: test-topic-2, Partitions: 4, ReplicationFactor: 7, BrokerAddress: broker:8090",
        String.valueOf(kafkaSpec));
  }
